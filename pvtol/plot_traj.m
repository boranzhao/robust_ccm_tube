...
if sim_config.include_obs == 0
    ...
    plot(xnomTraj_ccm(1,:),xnomTraj_ccm(2,:),'k:','Linewidth',line_wd);
...
if sim_config.include_obs == 1
    visualize_obs(obs); 
    annotation('arrow',[0.15 0.25],[0.6 0.6],'Linewidth',2);
    text(-3.8,7.5,'Wind','Fontsize',15);
    xlabel('$p_x$ (m)','interpreter','latex');
    ylabel('$p_z$ (m)','interpreter','latex')
else
    annotation('arrow',[0.15 0.25],[0.6 0.6],'Linewidth',2);
    text(-4.5,7.8,'Wind','Fontsize',15);
    xlabel('$p_x$ (m)','interpreter','latex');
    ylabel('$p_z$ (m)','interpreter','latex')
end
...
if sim_config.include_obs == 0
    ...
    legend([h2,h3,h4,h2_tube,h3_tube,h4_tube],{'CCM','RCCM','RCCM-P'},'Location','northwest');
end
goodplot([6 5]);
...
if sim_config.include_obs == 0
    ylim([6 12]);
    xlim([6 12]);
    text(6.2,9.8,'Wind','Fontsize',15);
    goodplot([6 3]);
else
    ylim([3 8]);
    xlim([0 15]);
    text(0.5,7.2,'Wind','Fontsize',15);
    goodplot([6 2.8]);
end
