function [alpha_opt,mu_opt,w_lower,max_residual] = rccm(plant,controller,W,dW_dt,Y,vars_WY,lambda,state_set)
...
if state_set.consider_state_set == 1
    ...
    for i=1:length(state_set.box_lim) 
        ...
        % Lagrange multiplier
        if class(Bw(state_set.W_states_index)) == "sdpvar" || norm(Bw(state_set.W_states_index,:)) > 0 
